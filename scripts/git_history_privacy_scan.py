...
EMAIL_PATTERN = re.compile(
    rb"(?i)\b[A-Z0-9._%+-]+@([A-Z0-9.-]+\.[A-Z]{2,}|example\.test)\b"
)
...
def main() -> int:
    ...
    for line in objects:
        ...
        label = path or object_type
        findings.extend(_scan_bytes(content, f"{label} ({object_id[:12]})"))

    identities = str(
        _git(root, "log", "--all", "--format=%an%x00%ae%x00%cn%x00%ce", text=True)
    ).splitlines()
